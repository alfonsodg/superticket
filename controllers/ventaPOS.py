...
def venta():
    ...
    if request.vars.boton:
        time.sleep(5)
        session.direccion=(db(db.puntoVenta.id==request.vars.puntoVenta)
        .select(db.puntoVenta.ALL))[0].direccionPuntoVenta
        session.distrito=(db(db.puntoVenta.id==request.vars.puntoVenta)
        .select(db.puntoVenta.ALL))[0].idDistrito
        session.puntoVenta=request.vars.puntoVenta
        for x in xrange(len(session.lista)):
            db.ticket.insert(

...
##if session.tipoUsuario<>2: redirect(URL(r=request,c='default',f='index'))
...
def index(): 
    if not request.vars.id: 
        ...
        id=request.vars.id
    session.lista=""
    session.cliente=session.idUsuario
    session.vendedor=5   
    session.desc=0
    session.pEnvio=0
    session.puntoVenta=''
    ...
    *[OPTION(x.zona.nZona,_value=x.zona.id)
    ...
    #sql=db.executesql('select(count(numeroAsiento)) \
    #from asiento where asiento.idZona='+str(id)+' group by FilaAsiento')
...
def venta():
    ...
    if request.vars.boton:
        ...
        for x in xrange(len(session.lista)):
            ...
            idVendedor=str(session.vendedor),
